...
class Configure
{
     ...
     *
     * @see \Phinx\CakeCore\Configure::load()
     * @var \Phinx\CakeCore\Configure\ConfigEngineInterface[]
     */
     ...
     * @param \Phinx\CakeCore\Configure\ConfigEngineInterface $engine The engine to append.
     * @return void
     */
     ...
     * @return bool Success
     * @throws \RuntimeException if the adapter does not implement a `dump` method.
     */
    public static function dump(string $key, string $config = 'default', array $keys = []): bool
    {
        $engine = static::_getEngine($config);
        if (!$engine) {
            throw new RuntimeException(sprintf('There is no "%s" config engine.', $config));
        }
     ...
     * @return \Phinx\CakeCore\Configure\ConfigEngineInterface|null Engine instance or null
     */
